...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-eth-network-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Ethernet Network Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/EthNetworkPolicies/%s", moid), expectedJSONTemplate, vars)
}
